...
public class ExampleController : MonoBehaviour {
    ...
    int currentRotationIdx = -1;
    int rotationRepeats;
    public int rotation1;
    public int rotation2;

    public float rotationAngle;                                    // rotation angle used in each trial
    ...
    public void GenerateExperiment(Session experimentSession)
    {
        ...
        rotation1 = Convert.ToInt32(session.settings["rotation_1"]);
        rotation2 = Convert.ToInt32(session.settings["rotation_2"]);
        ...
        //make the second alinged block
        int numAlignedTrials2 = Convert.ToInt32(session.settings["num_trials_aligned_reach_2"]);
        Block alignedReachBlock2 = session.CreateBlock(numAlignedTrials2);
        alignedReachBlock2.settings["trial_type"] = "aligned_2";
        alignedReachBlock2.settings["visible_cursor"] = true;
        alignedReachBlock2.settings["rotation"] = 0;
        ...
        rotatedReachBlock1.settings["rotation"] = rotation1;

        //nocursor 2
        //make the no_cursor blocks (open JSON file to check the correct names)
        int numNoCursorTrials2 = Convert.ToInt32(session.settings["num_trials_noCursor_reach_2"]);
        Block noCursorBlock2 = session.CreateBlock(numNoCursorTrials2);
        noCursorBlock2.settings["trial_type"] = "no_cursor_2";
        noCursorBlock2.settings["visible_cursor"] = false;
        noCursorBlock2.settings["rotation"] = 0;
        ...
        rotatedReachBlock2.settings["rotation"] = rotation2;

        //nocursor 3
        //make the no_cursor blocks (open JSON file to check the correct names)
        int numNoCursorTrials3 = Convert.ToInt32(session.settings["num_trials_noCursor_reach_3"]);
        Block noCursorBlock3 = session.CreateBlock(numNoCursorTrials3);
        noCursorBlock3.settings["trial_type"] = "no_cursor_3";
        noCursorBlock3.settings["visible_cursor"] = false;
        noCursorBlock3.settings["rotation"] = 0;
        ...
        //make a list if rotations to shuffle
        rotationRepeats = numRotatedTrials1 / numTargets / 2;


        //below: Pseudo random targets
    ...
    public void StartReachTrial(Trial trial)
    {
        ...
        // Get the rotation used for this trial
        if (!trialType.Contains("rotated"))
        { 
        rotationAngle = Convert.ToSingle(trial.settings["rotation"]);
        }

        //Pseudorandom target location and rotation for rotated trials
        if (shuffledTargetList.Count < 1)
        {
            shuffledTargetList = new List<int>(targetList);
            shuffledTargetList.Shuffle();

            if (Convert.ToString(trial.settings["trial_type"]).Contains("rotated"))
            {
                //when you move on to next set of targets, move onto the next rotation
                if (trial.numberInBlock == 1) {

                    //reset all the things
                    print("setting up rotation angle list");
                    rotationList = new List<int>();
                    for (int i = 0; i < rotationRepeats; i++) { rotationList.Add(rotation1); }
                    for (int i = 0; i < rotationRepeats; i++) { rotationList.Add(rotation2); }
                    rotationList.Shuffle();

                    currentRotationIdx = -1;
                }

                Debug.LogFormat("switched at trial {0}", Convert.ToString(trial.number));
                currentRotationIdx++;

                rotationAngle = rotationList[currentRotationIdx];
            }

        }

        // determine target location for this trial
